    ...
    <>
      {user?._id ? (
        <div id="app-container">
          ...
          <nav id="bottom-menu">
            <Link to={`/items/mine/{user._id}`}>
              <FaParachuteBox
                size={30}
                style={{ color: 'var(--icon-color-grey)' }}
              ></FaParachuteBox>
            </Link>
            <Link to={`/items`}>
              <FaMapLocationDot
        ...
        <>
          ...
          {showSignup ? (
            <form id="signup-form" onSubmit={handleSignup}>
              ...
              <input
                ...
                required={true}
              ></input>
              <button className="signup-button button-turqouise" type="submit">
                sign up
              </button>
            </form>
          ) : (
            <form id="login-form" onSubmit={handleLogin}>
              ...
              <input
                ...
                required={true}
              ></input>
              <button className="login-button button-turqouise" type="submit">
                login
              </button>
            </form>
          )}
          ...
          <button
            className="toggle-signup-button button-turqouise"
            type="submit"
            onClick={() => setShowSignup(!showSignup)}
          >
            {showSignup ? 'cancel sign up' : '...or sign up'}
          </button>
        </>
      )}
    </>
