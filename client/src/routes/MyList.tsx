...
function MyList() {
  ...
  useEffect(() => {
    async function filterAndSet() {
      const filteredList = list.filter(
        (elem) => elem.owner === user._id && elem.available
      );
  ...
  return (
    <>
      <h2>My List</h2>
      <button id="filter-button" onClick={toggleList}>
        {showSavedItems ? 'show open' : 'show saved'}
      </button>
      <div id="my-list-container">
        {!myList.length ? (
          <p>Nothing in your list 🥦🥦🥦</p>
        ) : (
          myList.map((elem) => <MyItem key={elem._id} item={elem}></MyItem>)
        )}
        <button
          id="add-button"
        ...
        {showAddForm ? (
          <AddForm setShowAddForm={setShowAddForm}></AddForm>
        ) : null}
      </div>
    </>
  );
